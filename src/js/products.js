export default class Products {
    ...
    createElement(parentEl, name, price, id) {
        parentEl.insertAdjacentHTML('beforeend', `
            <tr class="page__row page__row-tbody" data-id="${id}">
                <td class="page__cell">${name}</td>
                <td class="page__cell">${price}</td>
                <td class="page__cell">
                    <?xml version="1.0" encoding="UTF-8" standalone="no"?>
                        <svg class="page__pensil" viewBox="0 0 66 65" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                            <title>Pencil</title>
                    ...
                    <span class="page__delete">х</span>         
                </td>                      
            </tr>`
        );
    }
    ...
    redRaw(parentEl) {
        ...
        this.list.forEach(el => {
            this.createElement(parentEl, el.name, el.price, el.id);  
        }); 
    }
}
